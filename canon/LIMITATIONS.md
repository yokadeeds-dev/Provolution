...
12. **Gewichts-Sensitivitätsanalyse (durchgeführt).** Die systematische Variation der SEC-J-Gewichte über plausible Bereiche **liegt jetzt vor**: [`studies/SENSITIVITY_2026-05-30/SENSITIVITY_REPORT.md`](../studies/SENSITIVITY_2026-05-30/SENSITIVITY_REPORT.md) (reproduzierbares Skript, 20.000 Monte-Carlo-Gewichtsvektoren, seed=42). Ergebnis: in der plausiblen Nachbarschaft (±0,10) bewegt sich Ø SEC-J nur um < 0,5 Prozentpunkte, **kein** Verdict wechselt, Ranking stabil (Spearman ρ ≥ 0,94); der J-Veto ist gewichtsinvariant (min J = 0,72 → 0 Vetos); 21/25 Hebel sind unter praktisch jeder Gewichtung TRAGFÄHIG. **Offen bleibt**: der Abgleich mit präferenzbasierten Elicitation-Verfahren (AHP/SMART) und die **Input**-Sensitivität (Variation der S/E/C/J-Scores selbst, ≠ Gewichts-Sensitivität).

13. **Out-of-sample-Falsifizierung (illustrative Probe vorhanden, KEINE Validierung).** Innere Konsistenz (§9) ist nicht dasselbe wie ein prädiktiver Test. Eine **illustrative retrospektive Konkordanz-Probe** (Proof-of-Concept) liegt vor: [`studies/ILLUSTRATIVE_CASES_2026-05-30/`](../studies/ILLUSTRATIVE_CASES_2026-05-30/ILLUSTRATIVE_CASES_REPORT.md) — N=7 über die volle Skala (0,085–0,94): Großer Sprung 1958 (0,085, Katastrophe) · PKW-Maut (0,19, J-Veto, EuGH) · Open-House-Maskenverfahren 2020 (0,35, instabil — hohes J=0,80 rettet die gebrochene Ausführung S/E/C nicht) · ASFINAG (0,84) · Apollo (0,92) · Währungsreform 1948 (0,94). SEC-J trennt **monoton und konkordant** zum dokumentierten Ausgang (= *strukturelle* Übereinstimmung, **nicht** prädiktive Validität). Zusätzlich **Failure-Mode-Lokalisierung**: die gebrochene Achse sagt die Art des Scheiterns vorher (PKW-Maut bricht auf J → Veto → juristische Kassation; Open-House auf S/E/C → operativer Kollaps, kein Veto). **Offen benannt:** nicht verblindet, Single-Rater, N=7 (nicht powered), retrospektive Anwendung, framework-neutrale Fälle ≠ Climate-Lever-Feldtest. Ein **powered, verblindeter, prä-registrierter** Test (Roadmap im Report §7) bleibt die härtere nächste Stufe.

14. **Einordnung in MCDA / IAM / RDM (Entwurf vorhanden).** Die tabellarische Abgrenzung gegenüber AHP/ELECTRE/PROMETHEE, Integrated Assessment Models (DICE/FUND/PAGE) und Robust Decision Making (Lempert et al.) liegt jetzt als Reviewer-Supplement vor: [`canon/METHOD_POSITIONING.md`](METHOD_POSITIONING.md) (formalisiert Manuskript §6.2, ohne Neuheits-Alleinanspruch). **Offen bleibt** die volle Gewichts-Sensitivität (#12) und der englische ESG-Response-Text.

15. **Auswahlbegründung der Hebel (Entwurf vorhanden).** Die Aufnahme- und Ausschluss-Kriterien (warum diese 49; was bewusst zurückgestellt ist — Kernkraft, BECCS/DAC, großskaliges Geoengineering — und nach welchem Kriterium) sind jetzt dokumentiert: [`canon/LEVER_SELECTION.md`](LEVER_SELECTION.md). **Offen bleibt** das systematische PS-U-Scoring der ausgeschlossenen Optionen und eine vollständige *deduktive* Herleitung der Menge (bis dahin: historisch konsolidiert + Kriterien-Filter, challenge-bar).

16. **Double-Counting-Audit & Inter-Domain-Korrektur (teilweise offen).** Per-Domain-Overlap *ist* bereinigt (z. B. B07 Kreislauf „schluckt" B08–B12: −32 → ~−16 Gt), und der realistische Netto-Median (−43,2 Gt, Monte-Carlo) ist dokumentiert. Offen bleiben: ~~eine **Sankey-Visualisierung** der Carbon-Flows~~ (✅ [`studies/CARBON_FLOW_2026-05-30/`](../studies/CARBON_FLOW_2026-05-30/README.md): Brutto −87,1/−58,6 → Netto-Median −43,2, Overlaps bereinigt), ~~eine **Hebel-zu-Hebel-Konflikt-Matrix**~~ (✅ **Erst-Entwurf**: [`studies/CONSISTENCY_MATRIX_2026-05-30/`](../studies/CONSISTENCY_MATRIX_2026-05-30/CONSISTENCY_MATRIX_REPORT.md) — 44×44, 121 dokumentierte Kanten, materialisiert die ⊥-Relation auditierbar; nur B12 mit niedrigem C\*=0,50 = der Warnschwellen-Hebel. Systematische Konflikt-Elicitation noch offen), die **Inter-Domain-Rückkopplung** (dekarbonisiert die C-Domäne den Strom, ändert sich die B-Domänen-Materialvorkette — bisher nicht modelliert) die **YAML-Domain-Tag-Zuordnung** ist ✅ bereinigt (Tag-Renames in `co2_master.yaml` v1.3; Doku-Nachzug 2026-05-30, total-neutral — einziger Rest: F23/F24-Wert-Ownership, ebenfalls total-neutral). Lesart bis dahin: **−43,2 Gt = realistischer Netto-Wert, −58,6 Gt = gescreentes Potenzial-Ceiling** (beide in STATUS.md §2 getrennt geführt) — bei der Kommunikation gleichrangig nennen, nicht das Ceiling allein.
