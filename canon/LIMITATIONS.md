...
16. **Double-Counting-Audit & Inter-Domain-Korrektur (teilweise offen).** Per-Domain-Overlap *ist* bereinigt (z. B. B07 Kreislauf „schluckt" B08–B12: −32 → ~−16 Gt), und der realistische Netto-Median (−43,2 Gt, Monte-Carlo) ist dokumentiert. Offen bleiben: eine **Sankey-Visualisierung** der Carbon-Flows, eine **Hebel-zu-Hebel-Konflikt-Matrix** (materialisiert die ⊥-Relation auditierbar), die **Inter-Domain-Rückkopplung** (dekarbonisiert die C-Domäne den Strom, ändert sich die B-Domänen-Materialvorkette — bisher nicht modelliert) die **YAML-Domain-Tag-Zuordnung** ist ✅ bereinigt (Tag-Renames in `co2_master.yaml` v1.3; Doku-Nachzug 2026-05-30, total-neutral — einziger Rest: F23/F24-Wert-Ownership, ebenfalls total-neutral). Lesart bis dahin: **−43,2 Gt = realistischer Netto-Wert, −58,6 Gt = gescreentes Potenzial-Ceiling** (beide in STATUS.md §2 getrennt geführt) — bei der Kommunikation gleichrangig nennen, nicht das Ceiling allein.
